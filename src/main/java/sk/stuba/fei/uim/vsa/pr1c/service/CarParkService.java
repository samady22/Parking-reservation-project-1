package sk.stuba.fei.uim.vsa.pr1c.service;


import sk.stuba.fei.uim.vsa.pr1c.AbstractCarParkService;
...
public class CarParkService extends AbstractCarParkService {
    ...
    public Object createCarParkFloor(Long carParkId, String floorIdentifier) {
        ...
        try {
            ...
            CAR_PARK_FLOOR car_park_floor = new CAR_PARK_FLOOR(new CPF_ID(car_park.getId(), floorIdentifier));
    ...
    public List<Object> getCarParkFloors(Long carParkId) {
        EntityManager em = emf.createEntityManager();
        Query query = em.createQuery("select cpf from CAR_PARK_FLOOR cpf where cpf.car_park.carParkId=:cpId", CAR_PARK.class);
    ...
    public Object deleteCarParkFloor(Long carParkId, String floorIdentifier) {
        ...
        em.getTransaction().begin();
        try {
            Query query = em.createQuery("select cpf from CAR_PARK_FLOOR cpf where" +
                    " cpf.id.carParkId=:cpId and cpf.id.floorIdentifier=:floorId", CAR_PARK_FLOOR.class);
            query.setParameter("cpId", carParkId).setParameter("floorId", floorIdentifier);
    ...
    public Map<String, List<Object>> getParkingSpots(Long carParkId) {
        ...
        try {
            ...
            PARKING_SPOT parking_spot = (PARKING_SPOT) query.getSingleResult();
            Map<String, List<Object>> map = new HashMap<>();
            map.put(parking_spot.getCar_park_floor().getId().getFloorIdentifier(), list);
            return map;
        } catch (Exception e) {
            return new HashMap<>();
        }
    ...
    public Map<String, List<Object>> getAvailableParkingSpots(String carParkName) {
        ...
        try {
            ...
            if (list.size() != 0) {
                map.put(list.get(0).getCar_park_floor().getId().getFloorIdentifier(), (List<Object>) (List<?>) list);
            }
            return map;
        } catch (NoResultException e) {
            return null;
    ...
    public Map<String, List<Object>> getOccupiedParkingSpots(String carParkName) {
        ...
        try {
            ...
            list.removeIf(ps -> ps.getReservation().size() == 0);
            if (list.size() != 0) {
                map.put(list.get(0).getCar_park_floor().getId().getFloorIdentifier(), Collections.singletonList(list));
            }
            return map;
        } catch (NoResultException e) {
            return null;
    ...
    public List<Object> getReservations(Long parkingSpotId, Date date) {
        ...
        try {
            Query query = em.createQuery("select ps from PARKING_SPOT ps where ps.parkingSpotId=:psId", PARKING_SPOT.class);
            query.setParameter("psId", parkingSpotId);
            List<PARKING_SPOT> list = query.getResultList();
            if (!list.isEmpty()) {
                if (list.get(0).getReservation().size() != 0) {
                    list.removeIf(ps -> !isSameDay(ps.getReservation().get(0).getDate(), date));
    ...
    public static int hoursDifference1(Date startDate, Date endDate) {
        ...
        Duration total = Duration.ofMinutes(ChronoUnit.SECONDS.between(start, end));
        long hours = total.toHours();
        long seconds = total.minusHours(hours).getSeconds();
